#!/usr/bin/env python3
"""
Comprehensive performance test for real BLS12-381 library usage
"""
...
def test_real_bls12_381_performance():
    """Test real BLS12-381 performance and verify library usage"""
    print("=== Real BLS12-381 Performance Test ===")
    print("=" * 60)
    ...
    try:
        ...
        start_time = time.time()
        d = field.mul(a, b)
        mul_time = (time.time() - start_time) * 1000
        ...
        print(f"   📊 Performance (ms): Add={add_time:.4f}, Mul={mul_time:.4f}, Sub={sub_time:.4f}, Inv={inv_time:.4f}")
        ...
        print(f"✅ Field order: {field_order}")
        print(f"✅ All cryptographic operations using real BLS12-381")
        
        print(f"\n📊 PERFORMANCE METRICS:")
        print(f"   Field Operations (ms):")
        print(f"     - Addition: {add_time:.4f}")
        print(f"     - Multiplication: {mul_time:.4f}")
        print(f"     - Subtraction: {sub_time:.4f}")
        print(f"     - Inverse: {inv_time:.4f}")
...
if __name__ == "__main__":
    success = test_real_bls12_381_performance()
