...
class FullGKRProver:
    ...
    def sumcheck_init_phase2(self):
        ...
        self._init_beta_table(self.beta_g, cur.max_bl_u, self.r_u[self.sumcheck_id], 1)
            ...
            for i in range(1 << bit_length):
                ...
                for j in range(bit_length):
                    if isinstance(r_1, list):
                        ...
                        r_1_val = r_1  # Use the integer value directly
                    
                    if (i >> j) & 1:
                        beta_table[i] = self.field.mul(beta_table[i], 
                                                    self.field.add(alpha, self.field.mul(beta, r_1_val)))
                    else:
                        beta_table[i] = self.field.mul(beta_table[i], 
                                                    self.field.sub(1, self.field.add(alpha, self.field.mul(beta, r_1_val))))
...
def demo_full_hyrax_protocol():
    ...
    # Verify evaluation
    is_valid = hyrax_verifier.verify(x, evaluation)
    if is_valid:
